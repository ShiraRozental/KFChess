...
//
// Holding a raw Piece* per motion (see Motion) is safe under two invariants
// the caller (GameEngine) must uphold: only one move may be active at a
// time, and a move arriving at a still-jumping piece's cell is always
// intercepted rather than capturing it. Together these guarantee a piece
// with an active motion can never be removed from the board while that
